...
namespace NiNoKuni
{
	class SaveData
	{
		...
		public bool Open(String filename, bool force)
		{
			...
			{
				int count = 0;
				foreach(var ch in "ATEM")
				{
					if (tmp[count++] != ch) return false;
				}
			}
		...
		public bool Save()
		{
			...
			try
			{
				...
				Byte[] output = new Byte[comp.Length + mHeader.Length];
				Array.Copy(mHeader, output, mHeader.Length);
				Array.Copy(comp, 0, output, mHeader.Length, comp.Length);
